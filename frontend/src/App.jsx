...
import { apiFetch, getAuthSession, setAuthSession } from '@/lib/api';

export default function App() {
  // Signed login session persists across refreshes; null = Read-Only Public Tier.
  const savedSession = getAuthSession();
  const [activeTab, setActiveTab] = useState('overview'); // Default tab set to 'overview' (Dashboard)!
  const [currentRole, setCurrentRole] = useState(savedSession?.role || 'Read-Only Public Tier'); // Default role: Public Tier!
  const [loggedInUser, setLoggedInUser] = useState(savedSession?.username || '');
  ...
  const handleLoginSuccess = (uname, role, token) => {
    setAuthSession({ token, username: uname, role });
    setCurrentRole(role);
