...
def _reviewer_headers():
    """Signed MoSPI Reviewer token, minted directly — stateless, so the tests
    work against any users-collection state (fresh mock or legacy real DB)."""
    token = backend_auth.mint_token("Netai", backend_auth.ROLE_MOSPI_REVIEWER)
...
def test_human_review_workflow_and_rbac():
    ...
    # 1. Signed MoSPI Reviewer token succeeds
    review_payload = {
    ...
    # 2. Public Tier (no token) is forbidden
    pub_res = client.post(
        f"/api/works/{work_id}/review",
        json=review_payload,
    )
    ...
    # 3. The old client-supplied role header no longer elevates anyone
